<?php
   ...
   function addCustomer($fname, $lname, $address, $phone, $concern) {
      ...
      $sql ='INSERT INTO CUSTOMERS (FIRSTNAME, LASTNAME, ADDRESS, PHONE, CONCERN) VALUES ("'.$fname.'", "'.$lname.'", "'.$address.'", "'.$phone.'", "'.$concern.'");';
      $db->query($sql);
   }
   ...
   function addEvent($ename, $loc, $desc, $date, $time, $img) {
      ...
      $sql ='INSERT INTO EVENTS (EVENTNAME, LOCATION, DESCRIPTION, DATE, TIME, IMAGE) VALUES ("'.$ename.'", "'.$loc.'", "'.$desc.'", "'.$date.'",  "'.$time.'",  "'.$img.'");';
      $db->query($sql);
   }
